...
def process_html_file(in_path, out_path):
    ...
    for data in config.DYNAMIC_PAGES_CONFIG:
        if "reference_html" in data and data["reference_html"] == local_rel_path:
            file_data.is_searchable = bool(data["searchable"])
...
def render_file(file_data, template):
    ...
    utils.update_links_abs(bs4, os.path.dirname(file_data.in_path))
    content_dict = {'page_title': file_content["title"], 'main_content': get_body_content(bs4),
                    'body_class': file_data.body_class, str("section_" + file_data.section): "true"}
    # append file meta
    ...
    utils.update_links(bs4, PATHS["TEMPLATE_PATH"], file_data.in_path, file_data.out_path)

    if bs4 is None:
        log("Error generating file, so skipping: " + file_data.in_path, 2)
        return
